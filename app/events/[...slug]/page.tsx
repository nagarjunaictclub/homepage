...
export default function Page({
  ...
  searchParams?: { [key: string]: string | any } | Promise<{ [key: string]: string | any }>;
}) {
  // Support Next.js 13/14 sync params & Next.js 15+ async params
  ...
  // Lookup event from eventData JSON module
  const matchedEvent = eventData.find((item) => {
    if (!slugString) return false;
  ...
  // Extract properties from JSON data, falling back to query searchParams if provided
  const title =
    matchedEvent?.title ||
    ...
    "Event Details";
  const eventDate =
    matchedEvent?.eventDate || resolvedSearchParams?.eventDate || "TBA";
  const image =
    matchedEvent?.image ||
    resolvedSearchParams?.image ||
    "https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=1200&q=80";
  const description =
    matchedEvent?.description || resolvedSearchParams?.description;
  const objective =
    matchedEvent?.objective || resolvedSearchParams?.objective;
  const winning_price =
    matchedEvent?.winning_price || resolvedSearchParams?.winning_price;
  const registrationLink =
    matchedEvent?.registrationLink || resolvedSearchParams?.registrationLink;
  const giveaway =
    matchedEvent?.Giveaway || resolvedSearchParams?.Giveaway;
  const extra =
    matchedEvent?.extra ||
    (Array.isArray(resolvedSearchParams?.extra)
      ? resolvedSearchParams.extra
      : []);
  ...
  const EntryFeeAndRegistration = formatList(
    matchedEvent?.EntryFeeAndRegistration ||
      resolvedSearchParams?.EntryFeeAndRegistration
  );
  const ModeofOperation = formatList(
    matchedEvent?.ModeofOperation || resolvedSearchParams?.ModeofOperation
  );
  ...
  const instructorAddress = matchedEvent?.instructor_address;
  ...
  // Render 404 Fallback if event is completely missing
  if (
    !matchedEvent &&
    !resolvedSearchParams?.title &&
    ...
    return (
      <section className="px-4 py-32 text-center min-h-[60vh] flex flex-col items-center justify-center bg-gray-50 dark:bg-dark">
        ...
        <p className="text-body-color dark:text-gray-300 mb-8 max-w-md">
          We couldn't find the event details for "{decodedSlug}".
        </p>
        <Link
          href="/events"
